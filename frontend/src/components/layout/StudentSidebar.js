...
export default function StudentSidebar({ children }) {
  ...
  const activeItem = NAV.find(item => (item.to === '/student' ? location.pathname === '/student' : location.pathname.startsWith(item.to)));
    ...
    <div className="min-h-screen bg-brand-surface font-sans">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[272px] flex-col border-r border-white/70 bg-white/85 shadow-[0_24px_60px_rgba(145,158,171,0.16)] backdrop-blur-xl lg:flex">
        <div className="border-b border-brand-border/70 px-6 py-6">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-primary to-[#7e57c2] text-sm font-black text-white shadow-lg">BIT</div>
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-5 space-y-1 custom-scrollbar">
          {NAV.map(item => (
            <NavItem key={item.to + item.label} {...item} end={item.to === '/student'} />
          ))}
        </nav>
          ...
          <div className="fixed inset-0 z-50 flex lg:hidden">
            ...
            <div className="relative flex w-[272px] flex-col bg-white shadow-xl">
              ...
              <div className="flex-1 overflow-y-auto px-4 pb-5">
                {NAV.map(item => (
                  <NavItem key={item.to} {...item} end={item.to === '/student'} onClick={() => setMobileOpen(false)} />
                ))}
              </div>
              <div className="p-4 border-t border-brand-border/70">
                <button
